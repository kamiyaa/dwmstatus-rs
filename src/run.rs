...
fn read_to_string(p: &Path) -> io::Result<String> {
    let mut file = fs::File::open(p)?;
...
fn print_info(config: &AppConfig, sys: &systemstat::System) -> Result<(), std::io::Error> {
    let cpu_temp = if let Some(p) = config.cpu.temperature.as_ref() {
    ...
    let battery_charge = if let Some(p) = config.battery.percentage.as_ref() {
        match read_to_usize(p) {
    ...
    let battery_status = if let Some(p) = config.battery.status.as_ref() {
    ...
    if let Some(battery) = battery_charge {
        match battery_status.as_ref() {
            Some(status) => match status.as_str() {
                "Charging" => print!("[{:.1}%+] \u{2502} ", battery),
